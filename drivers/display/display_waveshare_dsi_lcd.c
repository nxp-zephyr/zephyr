...
static int waveshare_dsi_lcd_enable(const struct device *dev)
{
	const struct waveshare_dsi_lcd_config *config = dev->config;

	config->write_bus(dev, 0xad, 0x01);

	return 0;
}

static int waveshare_dsi_lcd_disable(const struct device *dev)
{
	const struct waveshare_dsi_lcd_config *config = dev->config;

	config->write_bus(dev, 0xad, 0x00);
