...
static int udc_mcux_driver_preinit(const struct device *dev)
{
	...
	if (config->usbphy_clock_dev && config->usbphy_clock_rate)
		clock_control_set_rate(config->usbphy_clock_dev, config->usbphy_clock_subsys, config->usbphy_clock_dev);
...
#define UDC_MCUX_CLK0_DEFINE(n) \
		...
		.usb_clock_rate = DT_INST_PROP_BY_IDX(n, clock_rates, 0),
...
#define UDC_MCUX_CLK1_DEFINE(n) \
		...
		.usbphy_clock_rate = DT_INST_PROP_BY_IDX(n, clock_rates, 1),
