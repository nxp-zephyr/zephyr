 ...
 *  @return Needed buffer size to match the requested L2CAP SDU MTU.
 */
#define BT_L2CAP_RT_FC_SDU_BUF_SIZE(chan, mtu) (BT_L2CAP_BUF_SIZE(BT_L2CAP_RT_FC_SDU_HDR_SIZE((chan)) + (mtu) + BT_L2CAP_RT_FC_SDU_TAIL_SIZE((chan))))
...
struct bt_l2cap_br_chan {
	...
	/** @internal TX windows */
	struct bt_l2cap_br_window tx_win[CONFIG_BT_L2CAP_BR_MAX_WINDOW_SIZE];
 ...
 *  @return Needed buffer size to match the requested L2CAP PDU MTU.
 */
#define BT_L2CAP_CLS_BUF_SIZE(mtu) BT_BUF_ACL_SIZE(BT_L2CAP_HDR_SIZE + BT_L2CAP_CLS_HDR_SIZE + (mtu))
