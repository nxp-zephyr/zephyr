...
#define BUF_SIZE 32768 /* 64 blocks worth of data for most SD cards */
...
static void test_read(void)
{
	...
	block_addr = 3;
	ret = sdmmc_read_blocks(&card, buf + 1, block_addr, 10);
...
static void test_write(void)
{
	...
	ret = sdmmc_write_blocks(&card, buf, block_addr, 10);
	zassert_equal(ret, 0, "Multiple block card write failed");

	/* Try a series of reads from the same block */
	block_addr = sector_count / 2;
	for (int i = 0; i < 10; i++) {
		ret = sdmmc_write_blocks(&card, buf, block_addr, 64);
	...
	block_addr = 3;
	ret = sdmmc_write_blocks(&card, buf + 1, block_addr, 10);
...
static void test_rw(void)
{
	...
	for (int i = 0; i < 10; i++) {
		...
		ret = sdmmc_write_blocks(&card, buf + SDMMC_UNALIGN_OFFSET,
			block_addr, 32);
		zassert_equal(ret, 0, "Write to card failed");
		memset(check_buf + SDMMC_UNALIGN_OFFSET, 0, 32 * sector_size);
		ret = sdmmc_read_blocks(&card, check_buf + SDMMC_UNALIGN_OFFSET,
			block_addr, 32);
		zassert_equal(ret, 0, "Read from card failed");
		zassert_mem_equal(buf + SDMMC_UNALIGN_OFFSET,
			check_buf + SDMMC_UNALIGN_OFFSET, 32 * sector_size,
			"Read of written area was not correct");
	}
}
