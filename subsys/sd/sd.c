...
static inline int sd_idle(struct sd_card *card)
{
	...
	cmd.response_type = SDHC_RSP_TYPE_NONE;
...
static int sd_send_interface_condition(struct sd_card *card)
{
	...
	cmd.arg = SDHC_VHS_3V3 | SDHC_CHECK;
	cmd.response_type = SDHC_RSP_TYPE_R7;
	cmd.timeout_ms = CONFIG_SD_CMD_TIMEOUT;
...
static int sd_init_io(struct sd_card *card)
{
	...
	/* TODO: SPI support will require chip select to be used */
	bus_io->chip_select = SDHC_CS_DONTCARE;
...
int sd_test_sdio(struct sd_card *card)
{
	...
	cmd.arg = 0;
	cmd.response_type = SDHC_RSP_TYPE_R4;
	cmd.timeout_ms = CONFIG_SD_CMD_TIMEOUT;
	ret = sdhc_request(card->sdhc, &cmd, NULL);
	if (ret) {
		/*
