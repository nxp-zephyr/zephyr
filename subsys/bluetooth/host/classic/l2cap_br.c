...
#define L2CAP_BR_IS_S_FRAME(flag) ((((uint32_t)flag) & L2CAP_BR_S_FRAME_FLAG_MASK) \
									== L2CAP_BR_S_FRAME_UD_FLAG)
...
#define L2CAP_EXTENED_FEAT_MASK	(L2CAP_FEAT_FIXED_CHAN_MASK \
								...
								| L2CAP_FEAT_RET_ENABLE_MASK \
								| L2CAP_FEAT_FC_ENABLE_MASK \
								| L2CAP_FEAT_ENH_RET_ENABLE_MASK \
								| L2CAP_FEAT_STREAM_ENABLE_MASK \
								| L2CAP_FEAT_FCS_ENABLE_MASK \
...
enum {
	...
	L2CAP_FLAG_RECV_FRAME_R,			/* Retransmission Disable (R) flag of received frame */
	L2CAP_FLAG_RECV_FRAME_R_CHANGED,	/* Flag the R flag changed.
					 * After send received frame with R bit set, clear the flag.
					 */
	...
	L2CAP_FLAG_SEND_FRAME_REJ_CHANGED,	/* Flag the REJ flag changed.
					 * After send received frame with REJ flag, clear the flag.
					 */
...
static int bt_l2cap_br_update_req_seq(struct bt_l2cap_br_chan *br_chan, uint16_t req_seq)
{
	int err = 0;

	if (bt_l2cap_br_check_req_seq_valid(br_chan, req_seq)) {
		err = bt_l2cap_br_update_req_seq_direct(br_chan, req_seq);
	} else {
...
static void l2cap_br_ret_timeout(struct k_work *work)
{
	...
	if (sys_slist_peek_head(&br_chan->_pdu_outstanding)) {
		if (br_chan->tx.mode == BT_L2CAP_BR_LINK_MODE_RET) {
			...
			bt_l2cap_br_update_req_seq(br_chan, expected_ack_seq);
			...
			bt_conn_tx_cb_t cb, void *user_data)
{
	...
	if (br_chan->tx.mode != BT_L2CAP_BR_LINK_MODE_BASIC) {
		if (net_buf_headroom(buf) < BT_L2CAP_RET_FC_SDU_CHAN_SEND_RESERVE(br_chan)) {
			/* Call `net_buf_reserve(buf,
			 * BT_L2CAP_RET_FC_SDU_CHAN_SEND_RESERVE(br_chan))` when allocating buffers
			 * intended for bt_l2cap_chan_send().
			 */
...
static void bt_l2cap_br_pack_s_frame_header(struct bt_l2cap_br_chan *br_chan, struct net_buf *buf,
					    uint8_t s)
{
	struct bt_l2cap_hdr *hdr;
	...
	pdu_len = BT_L2CAP_RT_FC_SDU_HDR_SIZE(br_chan) + BT_L2CAP_RT_FC_SDU_TAIL_SIZE(br_chan) - BT_L2CAP_RT_FC_SDU_LEN_SIZE;
	...
	if (s == BT_L2CAP_CONTROL_S_RR) {
		if (atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_P)) {
			f_bit = 1;
		} else if (atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P_CHANGED) &&
			atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
			p_bit = 1;
		} else if (atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY_CHANGED) &&
	...
	if ((br_chan->tx.mode == BT_L2CAP_BR_LINK_MODE_ERET) ||
		...
		if (br_chan->tx.extended_control) {
			...
			ext_control = sys_cpu_to_le32(
				BT_L2CAP_S_FRAME_EXT_CONTROL_SET(f_bit, br_chan->req_seq, s, p_bit));
			net_buf_add_le32(buf, ext_control);
		} else {
			/* Space occupied for enhanced control field. */
			enh_control = sys_cpu_to_le16(
				BT_L2CAP_S_FRAME_ENH_CONTROL_SET(s, p_bit, f_bit, br_chan->req_seq));
		...
		if (!br_chan->_pdu_buf) {
			...
			if ((atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R) &&
				...
				(atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY) &&
				 ...
				 atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_P) ||
...
static void l2cap_br_check_chan_config(struct bt_l2cap_br *l2cap, struct bt_l2cap_br_chan *br_chan)
{
	...
	case BT_L2CAP_BR_LINK_MODE_FC:
		if ((br_chan->rx.mode == BT_L2CAP_BR_LINK_MODE_FC) &&
			(!((l2cap->info_feat_mask & L2CAP_FEAT_FC_MASK) &&
			L2CAP_EXTENED_FEAT_MASK & L2CAP_FEAT_FC_MASK))) {
			__ASSERT(false, "Invalid mode %u", br_chan->rx.mode);
		...
		if ((br_chan->rx.mode == BT_L2CAP_BR_LINK_MODE_RET) &&
			(!((l2cap->info_feat_mask & L2CAP_FEAT_RET_MASK) &&
	...
	case BT_L2CAP_BR_LINK_MODE_FC:
		...
		break;
	case BT_L2CAP_BR_LINK_MODE_ERET:
		if (!((l2cap->info_feat_mask & L2CAP_FEAT_EXT_WIN_SIZE_MASK) &&
		    (L2CAP_EXTENED_FEAT_MASK & L2CAP_FEAT_EXT_WIN_SIZE_MASK))) {
		...
		if (!((l2cap->info_feat_mask & L2CAP_FEAT_FCS_MASK) &&
		    (L2CAP_EXTENED_FEAT_MASK & L2CAP_FEAT_FCS_MASK))) {
			      ...
			      struct net_buf *buf)
{
	...
	l2cap_br_check_chan_config(l2cap, br_chan);
				   ...
				   struct net_buf *buf, size_t len)
{
	...
	if (!((br_chan_sig->info_feat_mask & L2CAP_FEAT_QOS_MASK) &&
		(L2CAP_EXTENED_FEAT_MASK & L2CAP_FEAT_QOS_MASK))) {
...
static uint16_t l2cap_br_conf_rsp_opt_ext_flow_sepc(struct bt_l2cap_chan *chan,
				   struct net_buf *buf, size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
				...
				uint16_t opt_len, struct net_buf *buf)
{
	...
	while (buf->len >= sizeof(*opt)) {
		...
		case BT_L2CAP_CONF_OPT_EXT_FLOW_SPEC:
			result = l2cap_br_conf_rsp_opt_ext_flow_sepc(chan, buf, opt->len);
			if (result != BT_L2CAP_CONF_SUCCESS) {
				goto invalid_opt;
			}
			break;
				   ...
				   struct net_buf *buf, size_t len)
{
	...
	if (!((br_chan_sig->info_feat_mask & L2CAP_FEAT_EXT_FS_MASK) &&
		(L2CAP_EXTENED_FEAT_MASK & L2CAP_FEAT_EXT_FS_MASK))) {
		LOG_WRN("Unsupported extended flow spec");
		result = BT_L2CAP_CONF_REJECT;
		goto done;
	}
...
send_rsp:
	...
	if (BR_CHAN(chan)->tx.fcs == BT_L2CAP_BR_FCS_16BIT) {
		/* If peer enable FCS, local also needs to enable it. */
			     ...
			     uint16_t psm)
{
	...
	l2cap_br_check_chan_config(br_chan_sig, br_chan);
...
static void bt_l2cap_br_ret_fc_s_recv(struct bt_l2cap_br_chan *br_chan, struct net_buf *buf)
{
	...
	if (br_chan->rx.mode == BT_L2CAP_BR_LINK_MODE_ERET) {
		...
		atomic_clear_bit(br_chan->flags, L2CAP_FLAG_NEW_I_FRAME);
		atomic_clear_bit(br_chan->flags, L2CAP_FLAG_RET_REQ_I_FRAME);
	}
	...
	case BT_L2CAP_CONTROL_S_REJ:
		...
		if (br_chan->expected_ack_seq == req_seq) {
			/* ignore the REJ */
			LOG_DBG("ReqSeq %d equals ExpectedAckSeq, ignore REJ frame", req_seq);
			break;
		}
	...
	case BT_L2CAP_CONTROL_S_SREJ:
		...
		if (f) {
			if (atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_SREJ_ACTIONED) && (br_chan->srej_save_req_seq == req_seq)) {
...
static void bt_l2cap_br_ret_fc_i_recv(struct bt_l2cap_br_chan *br_chan, struct net_buf *buf)
{
	...
	err = bt_l2cap_br_update_req_seq(br_chan, req_seq);
	if (err) {
		return;
	}
...
valid_frame:
	...
	IF_ENABLED(CONFIG_BT_L2CAP_SEG_RECV, (if (br_chan->chan.ops->seg_recv) {
